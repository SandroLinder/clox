...
static void defineNative(const char* name, const NativeFn function) {
    ...
    tablePut(&vm.globals, AS_STRING(vm.stack[0]), vm.stack[1]);
    pop();
    pop();
}
...
static bool bindMethod(const ObjClass* klass, const ObjString* name) {
    ...
    ObjBoundMethod* bound = newBoundedMethod(peek(0), AS_CLOSURE(method));
    pop();
    push(OBJ_VAL(bound));
    return true;
}

static ObjUpvalue* captureUpvalue(const Value* local) {
...
static void defineMethod(ObjString* name) {
    ...
    tablePut(&klass->methods, name, method);
    pop();
}
...
static void concatenate() {
    const auto b = AS_STRING(peek(0));
    const auto a = AS_STRING(peek(1));
    ...
    ObjString* resutl = takeString(chars, length);
    pop();
    pop();
...
#define BINARY_OP(valueType, op) \
    ...
    for (;;) {
        ...
        switch (instruction = READ_BYTE()) {
            ...
            case OP_SUB: BINARY_OP(NUMBER_VAL, -);
                break;
            case OP_MULT: BINARY_OP(NUMBER_VAL, *);
                break;
            case OP_DIV: BINARY_OP(NUMBER_VAL, /);
                break;
            case OP_NEGATE: {
                if (!IS_NUMBER(peek(0))) {
            ...
            case OP_DEFINE_GLOBAL:
                ...
                break;
            case OP_GET_GLOBAL:
                const ObjString* varName = READ_STRING();
                Value value;
                ...
                break;
            case OP_SET_GLOBAL:
                const ObjString* existingName = READ_STRING();
                if (tablePut(&vm.globals, existingName, peek(0))) {
                    tableDelete(&vm.globals, existingName);
            ...
            case OP_SET_PROPERTY: {
                ...
                tablePut(&instance->fields, READ_STRING(), peek(0));
                Value value = pop();
                pop();
                push(value);
                break;
