...
static void errorAtToken(const Token* token, const char* message) {
    if (parser.panicMode) {
        return;
    }
...
static void binary(bool canAssign) {
    const TokenType opType = parser.previous.type;
    ...
    switch (opType) {
        case TOKEN_PLUS: emitByte(OP_ADD);
            break;
        case TOKEN_MINUS: emitByte(OP_SUB);
            break;
        case TOKEN_STAR: emitByte(OP_MULT);
            break;
        case TOKEN_SLASH: emitByte(OP_DIV);
            break;
...
static int addUpvalue(Compiler* compiler, const uint8_t index, bool isLocal) {
    ...
    if (upvalueCount == UINT8_COUNT) {
        errorAt("Too many closure variables in function.");
        return 0;
    }
...
static int resolveUpValue(const Compiler* compiler, const Token* name) {
    ...
    if (local != -1) {
        ...
        return addUpvalue((Compiler *) compiler, (uint8_t) local, true);
    }

    const int upvalue = resolveUpValue((Compiler *) compiler->enclosing, name);

    if (upvalue != -1) {
        return addUpvalue((Compiler *) compiler, (uint8_t) upvalue, false);
    }
...
static void addLocal(const Token name) {
    if (current->localCount == UINT8_COUNT) {
        errorAt("Too many local variables in function.");
        return;
    }
...
static void classDeclaration() {
    ...
    if (match(TOKEN_LESS)) {
        ...
        if (identifiersEqual(&className, &parser.previous)) {
            errorAt("A class can't inherit from itself.");
        }
...
static void returnStatement() {
    ...
    if (match(TOKEN_SEMICOLON)) {
        ...
        if (current->type == TYPE_INITIALIZER) {
            errorAt("Can't return a value form an initializer.");
        }
        expression();
...
static void super_(bool canAssign) {
    if (currentClass == NULL) {
        errorAt("Can't use 'super' outside of a class.");
    } else if (!currentClass->hasSuperclass) {
        errorAt("Can't use 'super' in a class with no superclass.");
    }
...
static void this_(bool canAssign) {
    if (currentClass == NULL) {
        errorAt("Can't use 'this' outside of a class.");
        return;
    }
    variable(false);
}
