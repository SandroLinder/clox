...
ObjBoundMethod* newBoundedMethod(Value receiver, ObjClosure* method) {
...
static ObjString* allocateString(char* chars, const int length, const uint32_t hash) {
    ...
    string->hash = hash;
    push(OBJ_VAL(string));
    tablePut(&vm.strings, string, NIL_VAL);
    pop();
...
ObjUpvalue* newUpvalue(const Value* slot) {
    ObjUpvalue* upvalue = ALLOCATE_OBJ(ObjUpvalue, OBJ_UPVALUE);
    upvalue->location = (Value *) slot;
